 ...
 */
function CJScroll(mcThis, mcMask, mcWarning) {
    m_mcScroll = mcThis;
    m_mcMask = mcMask;
 ...
 * @param {y좌표 위치} posY 
 */
CJScroll.prototype.scrollToPos = function (posY) {
 ...
 * @param {인덱스 번호} index 
 */
CJScroll.prototype.scrollAtIndex = function (index, bImmediately) {
 ...
 * 스크롤 사용성 설정
 */
CJScroll.prototype.setEnabled = function(b) {
 ...
 * 스크롤 메세지를 숨긴다
 */
CJScroll.prototype.hideWarning = function() {
...
function init() {
    ...
    m_stage.addEventListener("changeStep", function(e) {
        ...
        m_stage.removeChild(mcThumbMask);
        m_stage.removeChild(m_mcThumb);

    });

    m_nMinContentY = m_mcScroll.y;
    m_nMaxContentY = m_mcScroll.y - m_contentInfo.getHeight + m_maskInfo.getHeight;

    // 마스크 영역 설정
    var mask = new createjs.Shape();

    var pt = new createjs.Point(m_mcScroll.x, m_mcScroll.y);
    var ptStage = m_mcMask.localToGlobal( m_mcMask.parent.x, m_mcMask.parent.y );

    mask.graphics.rect(ptStage.x, ptStage.y, m_maskInfo.getWidth, m_maskInfo.getHeight);
    m_mcScroll.mask = mask;
    m_stage.addChild(mask);

    // 스크롤바 영역 설정
    m_thumbInfo.x = m_mcScroll.x + m_contentInfo.getWidth;
    m_thumbInfo.y = m_mcScroll.y;
    m_thumbInfo.width = 8;
    ...
    var mcThumbMask = new createjs.Shape();
    mcThumbMask.graphics.rect(m_thumbInfo.x, m_thumbInfo.y, m_thumbInfo.width, m_maskInfo.getHeight);
    m_mcThumb.mask = mcThumbMask;

    m_stage.addChild(mcThumbMask);
    m_stage.addChild(m_mcThumb);
    m_stage.addChild(pageBase.getGNBContainer());
...
function startScroll(event) {
    ...
    this.CJScroll.prototype.hideWarning();
...
function stopScroll(event) {
    ...
    var endY = m_mcScroll.y + m_scrollInfo.delta * 10;
